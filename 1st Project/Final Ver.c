...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    while (!quit)
    {
        ...
        if(i < siraYapIndex)
        {
            ...
            {
                int alanInt;
                if(alan - (int)alan >= 0.5)
                alanInt = alan + 1;

                printf("\nToplam Rezerv Deger: %d",alanInt*10);
                printf("\nToplam Platform Sayisi: %d",alanInt/2);
                printf("\nToplam Sondaj Sayisi: %d",alanInt);
                printf("\n\nToplam Platform Maliyeti: %d",alanInt/2*PlatformBrm);
                printf("\nToplam Sondaj Maliyeti: %d",alanInt*SondajBrm);
                printf("\nToplam Maliyet: %d",(alanInt/2)*PlatformBrm+alanInt*SondajBrm);
                printf("\nKar Miktari: %d",alanInt*10 - ((alanInt/2)*PlatformBrm+alanInt*SondajBrm));
            }
        }
