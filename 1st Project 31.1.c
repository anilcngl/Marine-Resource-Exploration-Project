...
void filledRect(SDL_Renderer *renderer, int x, int y, int w, int h) {
    SDL_Rect rect;
    rect.x = x;
    rect.y = y;
    rect.w = w;
    ...
    // Yarı saydam mavi rengi ayarla (R, G, B, A)
    SDL_SetRenderDrawColor(renderer, 0, 0, 255, 128); // Burada A değeri yarı saydamlığı ayarlar
...
void drawCoordinates(SDL_Renderer *renderer, int *koordinat, int numPoints, int scaleFactor) {
    ...
    for (int i = 0; i < numPoints - 2; i += 2) {
        ...
        // Çizginin içini doldur
        filledRect(renderer, x1, y1, x2 - x1, y2 - y1);
